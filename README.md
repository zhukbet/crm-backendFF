...
## Статус — Seq 1–56 з `support_crm_backlog.xlsx` (весь бекенд-скоуп M13 закрито)
...
Також додав **dev-логін** (`POST /api/auth/dev-login`, `{username}` → сесія, без Telegram) —
для локального тестування без реального бота; хардкодно вимкнено, якщо `NODE_ENV=production`.

## Закриті дірки API (звірка з розділом 6 ТЗ)

Порівняв фактичні контролери з API-контрактом розділу 6 — знайшов і закрив чотири прогалини:

- **`GET/POST /labels`** — сервіс існував (Seq 11), контролера не було.
- **`GET/POST /teams`, `POST/DELETE /teams/:id/members`** — не існувало взагалі, хоча
  `AgentTeam`/`TeamMember` в схемі є з самого початку, і без цього стратегії маршрутизації
  round_robin/least_busy не мають кого ротувати.
- **`POST /tickets/:id/mark-not-customer`** — `ExcludedSendersService.markTicketSenderAsNotCustomer`
  (Seq 24) був написаний ще на самому початку, але жоден контролер його не викликав — мертвий код.
- **`GET /tickets/:id/events`** — audit log (`ticket_events`) писався при кожній зміні тікета,
  але ніде не читався.

Усі чотири перевірені живцем на реальному Postgres: створив тікет через симульований webhook,
позначив «не клієнт» → тред закрився, наступне повідомлення того самого відправника коректно
проігнороване (`ignored-excluded`) — тредова логіка розділу 4 і excluded-senders працюють разом
правильно.
